...
class FPTree(object):
    ...
    def __str__(self):
        str_ = ''
        for node in self:
            str_ = str_ + "\n%s -> (%s)" % (node, [str(child) for child in node.children.values()])
        return str_
...
def build_fp_tree(transactions, min_support):
    ...
    # minimum support.
    item_frequencies = OrderedDict((item, support)
        for item, support in c.most_common()
        if support >= min_support)

    # function for sorting the items in the transaction
    sort_fn = lambda item: item_frequencies.keys().index(item)
